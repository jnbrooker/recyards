"""
app.py — Streamlit dashboard for the receiving-yards Monte Carlo model.

Run it with:
    streamlit run app.py

Pick a player, pick the opponent defense, set a prop line, and it simulates
the game thousands of times and shows the full distribution of receiving
yards plus the chance of going over the line.
"""
...
st.set_page_config(page_title="Receiving Yards Simulator",
                   page_icon="🏈", layout="wide")

# ---------------------------------------------------------------------------
# Cached data loaders (so the app is snappy after the first load)
# ---------------------------------------------------------------------------
...
def get_derived(seasons):
    ...
    return (model.list_players(wk),
            model.list_defenses(wk),
            model.team_pass_volume(wk),
            model.defense_profiles(wk))
...
seasons = st.sidebar.multiselect(
    "Seasons used to build priors",
    ALL_SEASONS, default=[2025, 2024],
    help="More seasons = steadier estimates; fewer = more current form. "
         "Recent seasons are weighted more heavily either way. A season that "
         "hasn't been played/published yet is skipped automatically.")
if not seasons:
...
# tell the user if any requested season wasn't available yet
...
line = st.sidebar.number_input("Prop line (receiving yards)",
...
y = sim["yards"]
cutoff = np.percentile(y, 99.5)          # trim the long tail for a readable axis
yv = y[y <= cutoff]

nbins = 60
counts, edges = np.histogram(yv, bins=nbins)
centers = (edges[:-1] + edges[1:]) / 2
probs = counts / len(y)                   # probability mass per bin
...
fig.update_layout(
    ...
    xaxis_title="Receiving yards in the game",
...
# ---------------------------------------------------------------------------
# Cumulative "chance of at least X yards" curve
# ---------------------------------------------------------------------------
