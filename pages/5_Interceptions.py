...
if view == "Game":
    ...
    game = UI.game_picker(ctx, seasons, recency, ['QB'], key='int')
...
if game is not None:
    ...
    typical_vol = pri["mu_att"]
    k = factors["dropbacks"][0] / max(factors["dropbacks_typical"], 1e-6)
    pri = RO.scale_volume_priors(pri, "mu_att", "var_att", pri["mu_att"] * k)
...
if factors is not None:
    st.caption(UI.script_caption(factors, game["game_row"], "attempts",
                                 factors["dropbacks_typical"], factors["dropbacks"][0], typical_vol, pri["mu_att"]))
if live_row is not None:
