...
import model
from nflsim import data as D, game as G
...
def get_weekly(seasons, recency):
    return model.load_weekly(tuple(sorted(seasons)), recency)
...
if view == "Game":
    ...
    game = UI.game_picker(ctx, seasons, recency, ['WR', 'TE', 'RB'], key='rec')
    ...
    if use_live:
        ...
        if rosters.empty:
            ...
            live_row = UI.pick_player(rosters, ['WR', 'TE', 'RB'], key='rec')
...
try:
    pri = model.player_priors(wk, player_id)
except ValueError:
    if live_row is None:
        raise
    pri = RO.receiving_priors_from_role(wk, live_row)
if live_row is not None:
    pri["team"] = live_row["team"]
    pri["mu_ts"] = UI.live_share(live_row, "target_share")
pos = pri["position"]
...
if game is not None:
    ...
    tv_typical = tv
    # this page's team volume is in TARGETS; move it by the game's dropback ratio
...
if live_row is not None:
    ...
    st.caption(UI.role_caption(live_row, "target_share", "targets"))
if factors is not None:
    st.caption(UI.script_caption(factors, game["game_row"], "targets", tv_typical[0], tv[0],
                                 tv_typical[0] * pri["mu_ts"], tv[0] * pri["mu_ts"]))
...
with left:
    ...
    st.dataframe(pd.DataFrame({
        "Metric": ["Target share", "Catch %", "aDOT (depth of target)",
                   "Air yards / catch", "YAC / catch", "Games"],
        "Mean":   [f"{pri['mu_ts']:.1%}", f"{pri['mu_catch']:.1%}",
                   f"{pri['mu_adot']:.1f}", f"{pri['mu_air']:.1f}",
                   f"{pri['yac_per_rec']:.1f}", str(pri["games"])],
        "Std (variance)": [f"±{pri['sd_ts']:.1%}", f"±{pri['sd_catch']:.1%}",
                           f"±{pri['sd_adot']:.1f}", f"±{pri['sd_air']:.1f}", "—", "—"],
